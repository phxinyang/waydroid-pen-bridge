...
class SwitchSafetyTests(unittest.TestCase):
    ...
    def test_integration_files_cover_optional_pro_lifecycle(self):
        ...
        self.assertIn("user-setup.sh", install)
        self.assertIn("disable --now", uninstall)
        self.assertIn("plasma/plasmoids", uninstall)
        self.assertIn("kwin/scripts", uninstall)
        ...
        # Prefer package-manager remove when rpm/deb installed the bridge.
        self.assertIn("detect_package_install", uninstall)
        self.assertIn("dnf remove", uninstall)
        self.assertIn("rpm -e", uninstall)
        self.assertIn("apt-get remove", uninstall)
        self.assertIn("dpkg -r", uninstall)
        self.assertIn("verify_clean", uninstall)
        ...
        user_setup = (ROOT / "user-setup.sh").read_text(encoding="utf-8")
        self.assertIn('widget.currentConfigGroup = ["General"]', user_setup)
        self.assertIn('widget.currentConfigGroup = ["General"]', uninstall)
        self.assertIn("org.xinyang.waydroidpenmode", user_setup)
        self.assertIn("waydroid-pen-mode@sheng", user_setup)
        self.assertIn("waydroid-pen-session.path", user_setup)
        self.assertIn("reset-failed", user_setup)
